...
def mono_t5_re_ranker(request: T5Request) -> ApiResponse:
    ...
    return generate_api_response(result=result.to_dict('records'),
                                 input=request.input,
                                 pipeline=f'pyterrier_t5.MonoT5ReRanker(batch_size={request.batch_size})',
                                 index_template='none',
                                 base_template='T5'
                                 )
...
def duo_t5_re_ranker(request: T5Request) -> ApiResponse:
    ...
    return generate_api_response(result=result.to_dict('records'),
                                 input=request.input,
                                 pipeline=f'pyterrier_t5.DuoT5ReRanker(batch_size={request.batch_size})',
                                 index_template='none',
                                 base_template='T5'
                                 )